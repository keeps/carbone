...
var preprocessor = {
  ...
  processQrcodes: function (template) {
    ...
    documents = [...documents, ...template.files.filter((x) => x.name.includes("word/header"))];
    documents = [...documents, ...template.files.filter((x) => x.name.includes("word/footer"))];
    let state = [];
      ...
      xml2js.parseString(document.data, (err, result) => {
        ...
        matches.map((match) => {
          ...
          if(fullUrl.includes('#qrcode')) {
            ...
            document.data = document.data.split('name="' + definition.name + '"').join('name="' + definition.name + '" qrcode="true"')
          }
        });
      });
    });
      ...
      xml2js.parseString(document.data, (err, result) => {
        ...
        matches.map((match) => {
          ...
          if(fullUrl.includes('#contains')) {
            ...
            document.data = document.data.split('name="' + definition.name + '"').join('name="' + definition.name + '" contains="true"')
          }
        });
      });
    });
  ...
  replateLocaleTagsDocx : function (template) {
    let document = template.files.find(
      (x) => x.name === 'word/document.xml'
    );
