...
var postprocessor = {
  ...
  replaceImageDocx: function (report) {
    ...
    documents = [...documents, ...report.files.filter((x) => x.name.includes("word/header"))];
    documents = [...documents, ...report.files.filter((x) => x.name.includes("word/footer"))];
    let state = [];
    ...
    documents.map((document) => {
      xml2js.parseString(document.data, (err, result) => {
        ...
        matches.map((match) => {
          ...
          var qrcode = definition.qrcode ? true : false;
