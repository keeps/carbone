  ...
  replaceImageDocx: function (report) {
    ...
    documents.map((document) => {
      xml2js.parseString(document.data, (err, result) => {
        ...
        var matches = xpath.find(result, "//w:drawing");
        ...
        matches.map((drawing) => {
          ...
          if(contains === 'true')
          {
            document.data = processImage(result, true, fullUrl, match, drawing, builder);
          }
...
function processImage(result, contains, fullUrl, match, drawing, builder) {
  if (contains) {
    let url = fullUrl;
    let dimensions = null;
